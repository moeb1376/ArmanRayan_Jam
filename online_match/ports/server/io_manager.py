...
class MySocket:
    ...
    def establish_connection(self):
        ...
        json_recv = self.socket_obj.recv(
            CONST.ServerSocket.MAX_DATA_RECV_SIZE).decode('ascii')
        try:
    ...
    def validation_password(self, recmsg):
        if recmsg.get("Password", CONST.IOMsg.DEFAULT_PASSWORD) != self.password:
            raise Exception("player", self.name, self.number,
...
class IOManager:
    ...
    def build_client(self):
        ...
        for i in range(CONST.ServerSocket.CLIENT_NUMBER):
            ...
            client_socket = MySocket(
                i + 1, random.randrange(1000, 9999), socket_obj, socket_address)
    ...
    def get_input(self):
        ...
        while round(time.time() - t, 3) <= CONST.ServerSocket.MAX_TIME_RECV and len(input_manager.my_task) == 0:
            continue
        print('Waiting time : ', round(time.time() - t, 3),
              time.time(), self.input_turn)
