...
class GraphicManager:
    ...
    def draw_menu_name(self, text):
        ...
        text_rect = text_surface.get_rect()
        center_temp = self.menu_height / \
            4 if len(self.name) == 1 else self.menu_height / 4 * 3
        text_rect.center = (self.menu_width / 2, center_temp)
        pygame.draw.rect(self.menu_surface, menu_main_color,
                         (0, center_temp - 50, self.menu_width, center_temp + 50))
        self.menu_surface.blit(text_surface, text_rect)
        pygame.display.flip()
        if len(self.name) == 2:
            text_surface = font.render(
                "Game Is Loading ...", 1, (244, 81, 30), menu_second_color)
            text_rect = text_surface.get_rect()
    ...
    def draw_points(self, points):
        ...
        self.surface.blit(text_surface, text_rect)
        text_surface = font.render("{:>5}".format(
            str(points[1])), 1, (255, 255, 255), (0, 0, 0))
        text_rect = text_surface.get_rect()
